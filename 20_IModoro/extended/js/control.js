...
function brechePomodoroAb() {
    ...
    // Pomodoro eintragen, nur wenn keine Pause gewesen war (sonst doppelt)
    if (!istPause) {
...
function timerTick() {
    ...
    let totalSekunden = 25 * 60; // 25 Minuten à 60 
    let aktuelleSekunden = (timer.getTimeValues().minutes * 60) + timer.getTimeValues().seconds; // (Aktuelle Minuten in Sekunden umwandeln) + Aktuele Sekunden
    let prozentFortschritt = 100 / totalSekunden * aktuelleSekunden; // Prozent in Miuten ausrechnen
...
function timerFinish() {
    // Pomodoro speichern, wenn keine Pause war (true, weil der Pomodoro erfolgreich beendet wurde)
    if (!istPause) {
    ...
    audioObject.onended = function() { // Füge eine Funktion zu einem Event hinzu
        ...
        if (istPause) { // wenn Pause ist, soll die Seite für den nächsten Pomodoro neu geladen werden
            location.reload();
        } else {
            istPause = true;

            // Titel verändern, da jetzt Pause ist
            document.querySelector('.timer__zeit').innerHTML = 'Pause!';

            // Grüner Balken signalisiert Pause
            document.querySelector('#pomoBalken').style.backgroundColor = "#05EB8B";
            startePomodoro(5)
        }
    };
}
